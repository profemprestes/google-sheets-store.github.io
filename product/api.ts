...
const api = {
  getProducts: async (): Promise<Product[]> => {
    return axios.get(googleSheetLink, { responseType: 'blob' }).then((response) => {
          ...
          complete: (results: Papa.ParseResult<unknown>) => {
            ...
            return resolve(
              products.map((product) => ({
                ...
                category:
                  (product.category as string) || (product.categoria as string) || DEFAULT_CATEGORY,
                description:
                  (product.description as string) || (product.descripcion as string) || '',
